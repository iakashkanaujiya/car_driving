...
describe('driving assistance', () => {
  ...
  it('auto-brakes when hand controls become inactive', () => {
    const plan = createSpeedPlan(
      { steering: 0, confidence: 0, active: false },
      20,
      GAME.maxSpeed,
      { targetSpeed: GAME.maxSpeed, leadDistance: Infinity, leadIsIncoming: false },
    );
  ...
  it('uses emergency braking for close oncoming traffic', () => {
    const plan = createSpeedPlan(
      { steering: 0, confidence: 1, active: true },
      25,
      GAME.maxSpeed,
      { targetSpeed: 5, leadDistance: 20, leadIsIncoming: true },
    );
